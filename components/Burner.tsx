...
const Burner: React.FC<BurnerProps> = ({ lit, onClick }) => {
  ...
  return (
    <group position={[0, -0.8, 1.5]}>
      <mesh 
        castShadow 
        onClick={(e) => { e.stopPropagation(); !lit && onClick(); }}
      ...
      <mesh position={[0, 0.2, 0]}>
         ...
         <meshStandardMaterial color={lit ? "#ff4400" : "#333"} emissive={lit ? "#ff2200" : "#000"} emissiveIntensity={3} />
      </mesh>
      
      {lit && (
        <>
          <pointLight position={[0, 0.4, 0]} distance={1.5} intensity={2} color="#ff5500" decay={2} />
          <ProceduralSmoke lit={lit} />
        </>
      )}
