...
const IncenseBurner: React.FC<IncenseBurnerProps> = ({ lit, onLight }) => {
  return (
    <div className="relative flex flex-col items-center justify-end h-64 w-40 translate-y-10">
      {/* Smoke Particles */}
      {lit && (
        <div className="absolute bottom-32 left-1/2 -translate-x-1/2 w-full h-full pointer-events-none">
          <div
            className="absolute left-1/2 bottom-0 w-2 h-2 bg-gray-400 rounded-full blur-sm animate-smoke opacity-50"
            style={{ animationDelay: "0s" }}
          ></div>
          <div
            className="absolute left-1/2 bottom-4 w-3 h-3 bg-gray-300 rounded-full blur-md animate-smoke opacity-40"
      ...
      <div
        className={`transition-all duration-1000 ${
          lit ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
        } flex gap-2 mb-2`}
      >
        <div className="w-1 h-24 bg-red-800 relative">
          {lit && (
            <div className="absolute -top-1 left-0 w-full h-1 bg-orange-500 animate-pulse rounded-full shadow-[0_0_10px_orange]"></div>
          )}
        </div>
      ...
      <button
        ...
        disabled={lit}
        className={`
          relative translate-y-4 w-32 h-24 bg-black rounded-b-[3rem] rounded-t-lg shadow-2xl border-t-4 border-yellow-600
