...
const MainHall: React.FC<MainHallProps> = ({ onBack }) => {
  ...
  return (
    <div className="w-full h-screen bg-black relative overflow-hidden select-none font-serif">
      ...
      {/* 3D Viewport */}
      <div className={`w-full h-full transition-all duration-1000 ${viewState !== 'idle' ? 'opacity-30 blur-sm' : 'opacity-100'}`}>
        <Canvas shadows camera={{ position: [0, 1.5, 5], fov: 40 }} dpr={[1, 2]}>
      ...
      <div className="absolute top-0 left-0 w-full p-8 flex justify-between z-10 pointer-events-none">
        ...
        <div className="flex flex-col items-end pointer-events-auto">
          <div className="text-[10px] text-amber-500/50 uppercase tracking-widest mb-1">Merit</div>
          <div className="text-3xl font-light text-amber-500 font-serif">{merit}</div>
        </div>
      </div>
      ...
      {floatingTexts.map(ft => (
         <FloatingText 
           ...
           y={ft.y} 
           text={ft.text} 
           onComplete={() => setFloatingTexts(prev => prev.filter(item => item.id !== ft.id))} 
         />
      ))}
      ...
      {buddhaImage && viewState === 'idle' && (
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-10">
           ...
           >
             <span className="group-hover:text-white transition-colors">Make a Wish</span>
           </button>
        </div>
      )}
      ...
      {viewState === 'praying' && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md animate-fade-in p-6">
           <div className="w-full max-w-md bg-[#0a0a0a] border border-stone-800 p-10 relative shadow-2xl">
              ...
              <h2 className="text-center text-amber-500 font-serif text-2xl tracking-[0.5em] mb-10">祈愿</h2>
              ...
              <div className="flex justify-center gap-8">
                 <button onClick={() => setViewState('idle')} className="text-stone-600 hover:text-stone-400 text-xs tracking-widest uppercase">Cancel</button>
                 <button 
      ...
      {viewState === 'result' && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-xl animate-fade-in p-6">
           <div className="max-w-xl text-center">
              <div className="text-amber-500/20 text-6xl mb-8 font-thin animate-pulse-slow">❖</div>
              <p className="text-stone-300 font-serif text-lg leading-loose tracking-wide mb-12">
                {blessing}
              </p>
              <button 
                onClick={() => { setViewState('idle'); setWish(''); }}
